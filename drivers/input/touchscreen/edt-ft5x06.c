...
static irqreturn_t edt_ft5x06_ts_isr(int irq, void *dev_id)
{
	...
	for (i = 0; i < MAX_SUPPORT_POINTS; i++) {
		...
		if (!down) {
           ...
           // dev_dbg_ratelimited(dev, "pressed at x:%d y:%d\n", ((x*800)/(tsdata->num_x*64)),((y*480)/(tsdata->num_y*64)));            
        }
					 ...
					 const struct i2c_device_id *id)
{
	...
	tsdata = kzalloc(sizeof(struct edt_ft5x06_ts_data), GFP_KERNEL);
    ...
    error = request_threaded_irq(EDT_IRQ_NR, NULL, edt_ft5x06_ts_isr,
				     IRQF_ONESHOT | IRQF_TRIGGER_RISING,
				     client->name, tsdata);

 //   error = request_irq(EDT_IRQ_NR, edt_ft5x06_ts_isr, 
...
//				     client->name, tsdata);
	...
	dev_dbg(&client->dev, "EDT FT5x06 initialized\n");
